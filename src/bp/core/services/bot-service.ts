...
export class BotService {
  ...
  async addBot(bot: BotConfig, botTemplate: BotTemplate): Promise<void> {
    ...
    const mergedConfigs = await this._createBotFromBot(bot, botTemplate)
    if (mergedConfigs) {
  ...
  private async _createBotFromBot(botConfig: BotConfig, source: BotTemplate): Promise<BotConfig | undefined> {
    const botSourcePath = path.join(process.PROJECT_LOCATION, `data/bots/${source.id}/`)
    const templateConfigPath = path.resolve(botSourcePath, BOT_CONFIG_FILENAME)

    try {
      const scopedGhost = this.ghostService.forBot(botConfig.id)
      const files = this._loadBotTemplateFiles(botSourcePath)
      if (fse.existsSync(templateConfigPath)) {
        const templateConfig = JSON.parse(await fse.readFileSync(templateConfigPath, 'utf-8'))
        const mergedConfigs = {
          ...DEFAULT_BOT_CONFIGS,
      ...
      this.logger.attachError(err).error(`Error creating bot ${botConfig.id} from bot "${source.name}"`)
    }
  }
